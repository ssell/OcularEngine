    ...
    namespace Graphics
    {
        ...
        bool D3D11GraphicsDriver::renderBounds(Core::SceneObject* object, Math::BoundsType const type)
        {
            ...
            if(object)
            {
                ...
                if(mesh && material)
                {
                    const auto aabb = object->getBoundsAABB(false);

                    const Math::Vector3f scaling = (aabb.getExtents() * 2.0f);
                    const Math::Vector3f offset = aabb.getCenter();
        ...
        void D3D11GraphicsDriver::getDepthRange(float* fnear, float* ffar) const
        {
            if(fnear)
            {
                *fnear = 0.0f;
